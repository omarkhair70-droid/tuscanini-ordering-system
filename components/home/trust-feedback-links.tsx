      ...
      <ul className="mt-4 grid gap-2.5 sm:grid-cols-3">
        {trustLinks.map((link) => (
          <li key={link.href}>
            <Link
              href={link.href}
              className="block rounded-xl border border-brand-red/20 bg-brand-white px-4 py-3 text-sm font-extrabold text-brand-red transition hover:border-brand-red hover:bg-brand-red/5"
            >
              {link.label}
            </Link>
          </li>
